...
shinyServer(function(input, output) {
  betas_fun <- reactive({
    ...
    big <-
      (sum(betas_small >= target, na.rm = TRUE) == 0 | maxn > 10000)
    return(list(
      betas_small = betas_small,
      betas_big = betas_big,
      big = big
    ))
  })
  ...
  betas_fun_clus <- reactive({
    ...
    betas_small <-
      apply(
        X = Ns_small,
        MARGIN = 1,
        FUN = power_calculator_cluster,
        mu_t = (60 + tau),
        mu_c = 60,
        sigma = sigma,
        ICC = ICC,
        n_clus_per_arm = n_clus_per_arm,
        alpha = alpha
      )
    betas_big <-
      apply(
    ...
    big <- (sum(betas_small >= target, na.rm = TRUE) == 0 |
              maxn > 10000)
    return(list(
      betas_small = betas_small,
      betas_big = betas_big,
  ...
  betas_fun_binary_clus <- reactive({
    ...
    betas_small <-
      apply(
        X = Ns_small,
        MARGIN = 1,
        FUN = power_calculator_binary_cluster,
        p0 = p0,
        p1 = p1,
        ICC = ICC,
        n_clus_per_arm = n_clus_per_arm,
  ...
  output$powerplot <- renderPlotly({
    if (input$clustered == FALSE & input$binary == FALSE) {
      ...
      results <- betas_fun()
      if (!results$big) {
        gp <- data.frame(n = Ns_small, power = results$betas_small) %>%
          ggplot(aes(n, power)) +
          geom_line(color = "blue") + xlim(0, maxn) + ylim(0, 1) +
            ggtitle(paste0("Power Analysis: Hypothetical Treatment Effect = ",
                           tau, " SD of outcome = ", sigma)) +
            ylab("Power (Probability of Statistical Significance)") +
            xlab("Number of Subjects") +
            geom_hline(yintercept = target, color = "red", lty = 2)
        ply <- ggplotly(gp)
        return(ply)
      }
        
      if (results$big) {
        gp <- data.frame(n = Ns_big, power = results$betas_big) %>%
          ggplot(aes(n, power)) +
          geom_line(color = "blue") + xlim(0, maxn) + ylim(0, 1) +
            ggtitle(paste0("Power Analysis: Hypothetical Treatment Effect = ",
                           tau, " SD of outcome = ", sigma)) +
    ...
    if (input$clustered == TRUE & input$binary == FALSE) {
      ...
      if (!results$big) {
        ...
        legend(
          "bottomright",
          legend = c(
            "Equivalent Individual-Level Design",
            "Clustered Design"
          ),
          col = c("blue", "green"),
          lwd = c(4, 4),
